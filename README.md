...
- **自选面板**：按 A 股 / 港股 / 美股分组，显示现价、涨跌、涨跌幅；美股盘前/盘后会标「盘前」「盘后」
- **搜索添加**：支持名称、代码、拼音/简拼，例如 `tx`、`pg`、`tsl`
- **整理自选**：左键固定/取消固定；右键进入排序，可拖动、上移、下移、删除
- **行情源回退**：腾讯 → 东方财富 → 新浪，约每 8 秒刷新
...
行情来自公开的第三方接口（腾讯、东方财富、新浪），仅供个人浏览，不构成投资建议，也不保证实时或准确。接口变更时应用可能暂时拿不到数据。
